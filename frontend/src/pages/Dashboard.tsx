...
export default function Dashboard() {
  ...
  // Filter beneficiaries by selected program's region
  const selectedProgram = programs.find(p => p.program_id === aidForm.program_id)
  // Workaround: Programs endpoint returns region name instead of ID right now, so we approximate or just show all for now.
  // Actually we can map region string to ID
  let targetRegionId = null;
  if (selectedProgram) {
    if (selectedProgram.region.includes("Mumbai")) targetRegionId = 1;
    else if (selectedProgram.region.includes("Pune")) targetRegionId = 2;
    else if (selectedProgram.region.includes("Nagpur")) targetRegionId = 3;
  }
  
  const eligibleBeneficiaries = beneficiaries
    .filter(b => !targetRegionId || b.location_id === targetRegionId)
    .sort((a, b) => {
  ...
  const handleDistributeAid = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAidForm({ program_id: '', beneficiary_id: '', resource_id: '', quantity_received: 1 })
    } catch (err: any) {
      console.error(err)
          ...
          <div className="space-y-4">
            {lowStockCount > 0 ? resources.filter(r => r.quantity_available < 100).map((r, i) => (
              <div key={i} className="p-4 border-l-4 border-red-500 bg-gradient-to-r from-red-50 to-white rounded-r-xl shadow-sm">
                ...
                <button className="text-xs font-bold bg-red-600 text-white px-4 py-1.5 rounded-lg hover:bg-red-700 transition-colors shadow-sm">Restock / Transfer</button>
              </div>
            )) : (
      ...
      <Modal isOpen={isAidModalOpen} onClose={() => setIsAidModalOpen(false)} title="Quick Action: Distribute Aid">
        <form onSubmit={handleDistributeAid} className="space-y-5">
          <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl mb-2">
            <h4 className="font-bold text-blue-800 text-sm mb-1">Step 1: Select Program</h4>
            <select required className="w-full px-3 py-2 border border-blue-200 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm bg-white" value={aidForm.program_id} onChange={e => setAidForm({...aidForm, program_id: e.target.value})}>
          ...
          <div className={`p-4 border rounded-xl transition-all ${aidForm.program_id ? 'bg-slate-50 border-slate-200' : 'bg-slate-100 border-slate-200 opacity-50'}`}>
            <h4 className="font-bold text-slate-800 text-sm mb-1">Step 2: Select Beneficiary</h4>
            <p className="text-xs text-slate-500 mb-3">Filtered by region. High priority beneficiaries are auto-suggested first.</p>
            <select required disabled={!aidForm.program_id} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm bg-white disabled:bg-slate-100" value={aidForm.beneficiary_id} onChange={e => setAidForm({...aidForm, beneficiary_id: e.target.value})}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1">Step 3: Resource</label>
